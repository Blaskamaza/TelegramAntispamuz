...
# ========== КОМАНДЫ БЕЗ ФИЛЬТРА InMainGroups (РАБОТАЮТ ВО ВСЕХ ГРУППАХ) ==========

@router.message(Command("ping", prefix="!/"))
async def cmd_ping_universal(message: Message) -> None:
    """Ping command - works in ANY group where bot is admin."""
    if message.chat.type not in ["group", "supergroup"]:
        return
    
    try:
        await message.reply("🏓 Pong! Bot ishlayapti")
        logging.getLogger(__name__).info(f"Ping from chat {message.chat.id}")
    except Exception as e:
        logging.getLogger(__name__).error(f"Ping error: {e}")


@router.message(Command("help", prefix="!/"))
async def cmd_help_universal(message: Message) -> None:
    """Help command - works in ANY group."""
    if message.chat.type not in ["group", "supergroup"]:
        return
...
**Egasi uchun:**
!spam - Spam sifatida belgilash
!stats - Statistika
"""
    
    try:
        await message.reply(help_text)
        logging.getLogger(__name__).info(f"Help from chat {message.chat.id}")
    except Exception as e:
        logging.getLogger(__name__).error(f"Help error: {e}")


@router.message(Command("me", "я", "info", "инфо", prefix="!/"))
async def cmd_me_universal(message: Message) -> None:
    """Show user info - works in ANY group."""
    if message.chat.type not in ["group", "supergroup"]:
        return
...
👤 Ism: {user_obj.first_name}
🆔 ID: {user_id}
⭐ Reytingi: {member.reputation_points if member else 0}
💬 Xabarlar: {member.messages_count if member else 0}
"""
        
        await message.reply(user_info)
        logging.getLogger(__name__).info(f"Me from user {user_id} in chat {message.chat.id}")
        
    except Exception as e:
        logging.getLogger(__name__).error(f"Me error: {e}")
        await message.reply("❌ Xatolik yuz berdi")

# ========== КОНЕЦ УНИВЕРСАЛЬНЫХ КОМАНД ==========
