...
async def check_for_unwanted(message: Message, msg_text: str, member: MemberData, tg_member) -> bool:
    ...
    # nsfw check (ALL profiles with low reputation)
    # Previously only female profiles were checked, but many spam accounts
    # don't have names or use neutral names, so we check everyone now
    if config.nsfw.enabled:
        # skip high-rep users (trusted members)
        if member.reputation_points > config.spam.allow_comments_rep_threshold:
            return False
