...
async def write_log(
    ...
    # Skip if no logs channel configured
    if not config.groups.logs or config.groups.logs == 0:
        logging.warning(f"[{log_type}] No logs channel configured, skipping log")
        return None
    ...
    try:
        ...
        logging.error(f"Failed to write log to channel {config.groups.logs}: {e}")
        return None
