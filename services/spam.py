...
_model_failed: bool = False  # flag to track if model loading failed
...
def predict(text: str) -> bool:
    ...
    # Skip ML if model loading failed previously
    if _model_failed:
        return False
    ...
    try:
        ...
        import logging
        logging.getLogger(__name__).warning(f"Spam model failed to load, disabling ML spam check: {e}")
        _model_failed = True
        return False
